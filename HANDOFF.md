...
## 26/9: Sidebar course header block (from the user's two reference screenshots)
- Inside a course, Teacher and Student sidebars now switch to that course: full-bleed header (`SidebarCourseHeader`: "Back to main" link, course **code** big, course name, "Section N" pill) then the course's pages at one item size (no more small/large split, no "Main" list inside a course — "Back to main" returns to `/teacher/courses` / `/student`). Outside a course: "Main" caption + top-level items, as before.
- Active item = tinted pill + a bar on the sidebar's outer edge (`after:-right-3`, the reference's right-edge bar). Item icons forced to 16px via `[&>svg]:w-4 [&>svg]:h-4`, so the per-icon `width` attrs (14px) no longer matter.
- Kept the navy/teal chrome theme instead of the reference's white sidebar + blue header (the header is a `bg-white/10` block, the section pill is white on the sidebar colour) so the theme toggle still works. Not done: the reference's expandable groups with nested sub-items — our course pages are a flat list.
- Removed `small` prop, `SidebarCourseName`, `SIDEBAR_DIVIDER`. `StudentShell` now passes `code` + `sectionNumber` to `StudentSidebar`. No code on a course → the name becomes the big line; no sectionNumber → no pill.
- Tests: `tests/sidebar-parity.spec.ts` rewritten (inside a course: same header/item metrics, code bigger than name, back link targets, no "Main"; outside: same width/items). `teacher-p4` details test now scopes the course code to `main` (it also appears in the sidebar). Suite 397 passed / 30 skipped; lint same as baseline.

## 26/9: Admin Courses — grouped by curriculum → subject, filters, no schedule/room, 37-course mock
- `admin/courses/page.tsx`: the flat Active/Archived lists are replaced by **curriculum groups** (collapsible; program chip + version label + counts) → **subject cards** (one card per subject/template: name + code + "N sections" + a "+ Section" button) → **section rows** (Sec chip, year·term chip, Archived badge, teachers, student count, edit/archive/delete). A subject's sections and its earlier terms therefore always sit together, newest offering first (`compareOfferings`: year ↓, term ↑, section ↑), and same-named subjects in different curricula (UX/UI in CE / CECS / CEI) stay apart. Ordering/grouping lives in `src/lib/courseGroups.ts` (pure functions). Curricula: CE, CECS, CEI, newest version first, "No curriculum" last; unlinked hand-made courses stack by code+name.
- Filters: search (name / code / teacher), Program, Curriculum, Term, Status, "Clear filters", "Collapse/Expand all", live count line ("37 sections · 24 subjects · 8 curricula"). Option lists come from what exists; a curriculum picked earlier that the program filter hides falls back to "all" (derived, no effect+setState). While any filter is on, every group is forced open.
- **Schedule / Room removed** from the admin page (edit popup block + the row subtitle). The fields stay on `Course` — the teacher pages still use them.
- "+ Section" moved from each row to the subject header; it copies the newest *active* offering and is absent when the subject is all archived (same rule as before). Row's expand button keeps the course name in its aria-label (`Name · Sec 1 · 2569 · Term 1`) since the name now lives in the card header (tests find rows by it).
- Mock data (`courses-mockup(-en).json`, `teachers-mockup(-en).json`, public + test-data byte-identical): 6 → 37 courses, 5 → 8 teachers; generated by a one-off script, c-mock-1..6 and t-mock-1..5 unchanged (student-flow mocks point at them). Cases: multi-section subjects, same subject across terms/years, term 3, no-teacher sections, TA + teacher on one section, same name in 3 curricula, archived-only curricula, hand-made courses with a long name and with no term info. New console command **[11]** in `test-data/seed-commands.txt` (TH + EN) loads curriculum + courses + teachers only.
- Tests: `tests/admin-courses-groups.spec.ts` (11). Suite 409 passed / 30 skipped; tsc + lint at baseline.
- Not done: no persisted collapse/filter state (resets on reload); the Add Course popup's curriculum picker still lists only open (no `effectiveTo`) curricula, so an old-curriculum offering can be created only by "+ Section" on an existing one.

## 26/9: Language toggle is back
...
## 26/9: the three open offers — Exam form controls, Thai leftovers, student TERM colour
- **Exam form** (`assignments/new` + `[assignmentId]/edit`): with Exam on, Accept Files / file types / Submission Type are replaced by a one-line note ("Students don't submit an exam — you enter everyone's score on the grading page"). Saved exam is always `acceptsFiles: false, fileTypes: [], submissionType: "individual", maxGroupSize: null` (a group setting picked before switching Exam on can't leak in), and Create no longer needs a file type when Exam is on.
- **Dates follow the language button**: new `lib/dateLocale.ts` (`dateLocale(lang)`). Student pages had `"th-TH"` hard-coded (Thai dates in English mode) and teacher assignments / grading / assignment detail had `"en-US"` hard-coded (English dates in Thai mode) — those 10 call sites now use it (the `fmtDate` helpers take `lang`). Pages that were already language-aware are unchanged; `lib/notifications.ts` (bell is hidden) still `en-US`.
...
## 26/9 (later): admin Students — status is now Active / Inactive tabs, not a select
- Replaces the Status dropdown from earlier today: a `PillTabBar` "Active (n) | Inactive (n)" under the Program/search row, same pattern as Enrolled/Withdrawn and Active/Archived. **Active is the default**; Inactive students only appear after clicking the Inactive tab. Counts follow the Program filter and the search; page resets to 1 on a tab change. Deactivating a student makes the row leave the Active tab (its count drops, Inactive's rises); Activate brings it back. Empty Inactive tab (no search) says "No inactive students". The tab bar is hidden only when there are no students at all.
- Tests: `tests/admin-students-status-filter.spec.ts` rewritten for the tabs (5). Suite 431 passed / 30 skipped; tsc clean.

## 26/9 (redesign): admin Students — one "Show inactive" switch instead of a second tab bar
- The Active/Inactive `PillTabBar` sat right under the Teachers/Students tabs — two tab bars stacked, and the user found it redundant. Replaced by a single **"Show inactive (n)" switch** (`role="switch"`) in the filter row after the Program filter. Default off = only active students (one click away from seeing inactive ones, as asked). On = inactive students are listed **after** the active ones (stable: active first, then inactive, each in the chosen sort order) and dimmed (`opacity-70`) with the Inactive badge. The count follows Program + search; the switch is disabled at 0. If a program has only inactive students and the switch is off, the empty state says so and points at the switch instead of "No results".
...
- (26/9, same feature) The batch popup now names programs in full (no CE / CECS / CEI anywhere in it) and has a **Program** select under the Batch select: "All programs — n" (default) or one program with its count, so a batch can be closed one program at a time. The chosen program falls back to "all" when the batch picked next does not have it; the "Deactivated …" notice adds the program name when one was chosen; Undo is unchanged. Tests: 2 more (one program at a time; fallback), file total 12. Suite: see the commit.

## 26/9: students mock data, batches 66–69 (console command [12])
- New `students-mockup.json` / `students-mockup-en.json` (public/mock-data + test-data, byte-identical; generated once by a scratch script): **57 cohort students** = the 13 of `student-flow-mockup` unchanged (same `cs-mock-1..13` ids / studentIds / names — [5]'s enrolments and logins still point at them) + **44 new**: batch 66 (12, 2 inactive — the batch about to graduate), 67 (12, 2 inactive, one with no title), 68 (13, 1 inactive), 69 (20, 1 inactive), each in CE / CECS / CEI. Ids like 660101NN / 670102NN / 680202NN / 690702NN, email = `studentId@kmitl.ac.th`; none collide with `students_sample.csv` ([3b]) or `course-import-test.csv` ([7]), so the import demos still work. Only `hwai_cohort_students_v1` is written. **Command [12]** in `test-data/seed-commands.txt` (TH + EN) — I ran the exact TH command in a browser page: 57 stored, lang th, table renders. Running [5] afterwards puts the list back to its 13.
- Side effect found by this data: the Students table's Title column (70px) cut "นางสาว" to "นาง…" and Thai names were clipped. Columns rebalanced (ID 108 / Title 92 / Name 18% / Email 18% / Program 19% / Status 96 / Actions 128): Title and the 3 action icons are complete at 1280px; a few long Thai surnames and the email tail still ellipsise at 1280 (fine from ~1440). Program can take 3 lines at 1280 (test allows ≤3, never clipped).
- Tests: 2 more in `tests/admin-students-status-filter.spec.ts` (file consistency + the flow mock's 13 unchanged; the popup lists 66/67/68/69 with 10/10/12/19 active). Suite 440 passed / 30 skipped expected; tsc clean.

## 26/9: admin Students — the Status column header sorts (like Student ID / Name)
- The user pointed at the Status column and asked for the same kind of control as Student ID and Name — those headers are click-to-sort (`SortableTh`), so Status now is too. Cycle: **Active first → Inactive first → back to the default (Student ID)**; `aria-sort` follows. It sorts by status, then by ID. Because a status sort is meaningless while inactive students are hidden, the **first click also switches "Show inactive" on** (only when the switch is off; it does nothing else magic — click again to go on, and the switch can still be turned off by hand). Default order (no status sort) is unchanged: active first, then inactive.
- Interpreted "filter" as sort because that is what those two headers do; the "Show inactive" switch remains the filter. If a dropdown filter on the header itself was meant, say so.
- Only StudentsTab: the Teachers table's Status header is not sortable (an earlier script hit it by mistake and was reverted before commit).
- Tests: 3 more in `tests/admin-students-status-filter.spec.ts` (cycle incl. the switch turning on, another column replaces it, no inactive people). Suite: see commit.

## 26/9: student sees what each rubric criterion earned; mock has rubric scores
- **Student classwork detail** (`student/courses/[secId]/classwork/[actId]`): once the assignment is graded *and announced*, the "Grading Rubric" card becomes "Grading Rubric — your points" and each criterion shows **earned / max** ("56 / 60", weight underneath) with a progress bar (`role="progressbar"`, aria-valuenow/max). Only real `Submission.criterionScores` are shown — with none saved (teacher entered a total only) the card stays the plain read-only rubric, no invented numbers. Before the results are announced nothing shows (the submission is already masked by `studentVisibleSubmission`). The Evaluation-page popup already read stored per-criterion scores; with the mock now carrying them it no longer shows the "Estimated from the criteria weights" warning.
- **Mock**: the 7 "graded" submissions of `student-flow-mockup(-en).json` (public + test-data, still byte-identical) each got `criterionScores` — integers, within each criterion's max, summing to `instructorScore` (e.g. Python exercise 90 = 56 + 18 + 16). Spliced into the hand-formatted lines as text (14 changed lines per file), not re-serialised. New console command **[13]** in `test-data/seed-commands.txt`; it is the same one-liner as [6] (writes only `hwai_submissions_v1`, so it does not undo the 57-student list from [12]) plus an English version. Ran all three in a browser page: 7 submissions with criterionScores each.
- Not done: teacher-side grading of the mock exams (no rubric, total only); per-criterion *comments* to students (the teacher's per-criterion note is "Feedback to AI" and hidden from students by design).
- Tests: `tests/student-rubric-points.spec.ts` (6: points + bars, no scores → plain rubric, hidden before announcing, Thai, Evaluation popup exact, mock invariants for all four files). Suite: see commit.

## 26/9: student My Courses = this term; "Completed courses" apart, by term, with the result
- `student/courses`: **"Current courses"** (cards, as before) = enrolled courses of the current term, plus a later term the student is already in, plus courses with no year/term at all (older records — nothing enrolled may disappear). **"Completed courses"** = the term has ended, an admin archived the course, or the student withdrew (badge "Withdrawn", no grade). Grouped by term, **newest first** ("Term 1/2569 · 1 course", then 2/2568, …); each row = cover icon, name, code · Sec · instructor, and the **final total % + grade letter** (`gradeLetter`, same tone chips as Evaluation). The total is the same weighted math as the Evaluation page (`computeCategoryGradeRows`/`computeTotalSoFar`) over the *announced* scores only (`studentVisibleSubmission`), normalised over the graded categories; nothing graded → "No grade yet" (no invented grade). A row opens the course like any other.
- **Current term = today's date** on the usual Thai calendar (`lib/academicTerm.ts`: Aug–Dec term 1, Jan–May term 2 of the academic year that began the previous August, Jun–Jul term 3; `summer` ranks with 3). The user explicitly deferred the real term system ("term system later" — an admin-maintained academic calendar with start/end dates, which is how registrars / Canvas / Moodle do it). Only `currentAcademicTerm` would need to read it; every screen goes through that file.
- **Student home**: the "My Courses" box, upcoming work and announcements now cover current-term courses only (past/withdrawn ones contribute nothing), with a "View all" link to `/student/courses`.
- **Mock**: `student-history-mockup(-en).json` (public + test-data): student **68020101** (Phurich Wongsawat) — this term c-mock-1; finished 2568: Programming 2568/1 (87.6% A), Programming 2568/2 (62.7% C), DSA 2568/1 (73.4% B), each 2 coursework + 1 exam, all announced; and c-mock-6 withdrawn. Console command **[14]** *appends* to what is in localStorage (skips ids already there), so it can be re-run and leaves [5]/[12] alone; needs [2] (37-course file) and 68020101 in the cohort ([5] or [12]). Verified by running the real [14] text in a browser page.
- Tests: `tests/student-my-courses-terms.spec.ts` (12; clock pinned with `page.clock`): term calendar boundaries, past/current/future/unknown/archived, the page (sections, grouping/order, results incl. unannounced and withdrawn, term rolling over in January, Thai), home box, mock consistency + results. Suite 461 passed / 30 skipped; tsc clean; lint on touched files unchanged (2 pre-existing).
- Not done / ideas: filter by term when many terms; per-term summary (credits, GPA — no credit data exists); a countdown/"week n of 16" for the current term (needs the calendar).

## 26/9 (later): richer term-history mock for the student My Courses page
- Asked for "more mock so it can be seen". `student-history-mockup(-en).json` now covers **three students**: **66010101** (4th year: 3 courses this term + 8 finished courses over 4 terms 2566/1–2567/2, grades A A B B C D A A), **67010201** (3rd year: 3 this term + 7 finished over 4 terms 2567/1–2568/2 incl. a D, an **F (47%)** and one course whose results are **not announced** → "No grade yet"), **68020101** (unchanged: A/C/B, one withdrawn, one this term). 69070101 (the main demo login) deliberately has no history — the first-term case (no "Completed courses" section).
- `courses-mockup(-en).json` 37 → **52** courses: 15 new *archived* offerings of the CE 2564 subjects (Programming, Discrete, DSA, Digital Logic, Architecture, OS, Database, SE) for 2566/2567/2568, given to the existing teachers (`teachers-mockup` updated). Side effect on /admin/courses: those subjects now show earlier terms inside the same subject card, and "Database Systems" exists in two curricula (CE 2569 and CE 2564) — `admin-courses-groups` expectations updated (52 sections · 29 subjects · 8 curricula; archived 22 · 12 · 4; the code search 4 · 2 · 2; the collapse test pins the CE 2569 card).
- Console command **[14]** is now one command: courses (52) + teachers + student list (57) overwrite, then the history is *appended* (skips ids already there, so it does not undo [5]). Ran the real English command in a browser page: 52 courses, 57 students, and 66010101's My Courses renders 3 cards + 4 term groups.
- Tests: `student-my-courses-terms` mock block rewritten (15 tests in the file): consistency for all three students (enrolments point at real courses, current = active 2569/1, finished = archived < 2569, exactly one unannounced course, weights = 100), the grades of all three, and the real merged data rendered for 66010101 / 67010201 / 69070101. Suite 464 passed / 30 skipped; tsc clean.
